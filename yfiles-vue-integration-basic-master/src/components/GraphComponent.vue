...
// Load all JSON files in yfiles_graphs (not only *_yfiles.json)
const graphModules = import.meta.glob('../../yfiles_graphs/*.json', {
  eager: true,
  import: 'default'
...
function buildManifestEntries(manifestEntries: ManifestEntry[] = []): ManifestEntry[] {
  ...
  for (const [key, payload] of Object.entries(graphModules)) {
    ...
    // Skip manifest.json (not a graph dataset)
    if (file.toLowerCase() === 'manifest.json') {
      continue
    }
...
async function ensureEvaluationSummaryLoaded(): Promise<void> {
  ...
  try {
    const response = await fetch('evaluation_summary.csv', { cache: 'no-cache' })
    if (!response.ok) {
    ...
    if (headerParts.length >= 3 && headerParts[1].includes('schema')) {
      startIndex = 1
    }
    for (let i = startIndex; i < lines.length; i++) {
      ...
      const [nameRaw, schemaRaw, relationshipRaw] = parts.map(part => part.trim())
      if (!nameRaw) {
        continue
      }
